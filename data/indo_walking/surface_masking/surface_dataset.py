...
class SurfaceSegmentationDataset(Dataset):
    ...
    def __init__(self, data_list: List[Dict], target_size: Tuple[int, int], is_train: bool):
        ...
        self.image_transform = transforms.Compose([
            ...
            # transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1) if self.is_train else transforms.Lambda(lambda x: x),
            # transforms.RandomHorizontalFlip(p=0.5) if self.is_train else transforms.Lambda(lambda x: x),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ])

        # 마스크 변환 (크기만 조절, Augmentation 없음)
        self.mask_transform = transforms.Compose([
            transforms.ToPILImage(),
            # 라벨 보존을 위해 NEAREST 보간법 사용
...
if __name__ == "__main__":
    ...
    try:
        ...
        # visualize_sample(dataset=valid_dataset, idx=0, metadata=metadata)

    except FileNotFoundError:
        print(f"🚨 오류: '{METADATA_PATH}' 파일을 찾을 수 없습니다.")
        print("전처리된 데이터 폴더 경로가 올바른지 확인해주세요.")
